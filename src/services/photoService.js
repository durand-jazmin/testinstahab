...
export const savePhotoService = async (img, width) => {
   ...
   const watermarkPath = 'src/util/oie_transparent.png'; // Ruta de la marca de agua
   const watermarkBuffer = await fs.readFile(watermarkPath);

   const watermarkedImage = await sharpImg.composite([
       { input: watermarkBuffer, gravity: 'southeast' } // Posición de la marca de agua
   ]);

   await watermarkedImage.toFile(imgPath);
